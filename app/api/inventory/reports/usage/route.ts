 ...
 * Uso por insumo: Inicial + Compras − Final = Uso, con extensión a costo.
 */
export async function GET(req: NextRequest) {
    try {
        ...
        // Reportes por sucursal: un rol corporativo debe elegir una.
        const scope = resolveBranchScope(user.role, user.branchId, validated.branchId);
        if (scope.kind === "NONE" || scope.kind === "ALL") {
            return NextResponse.json(
                { error: "Selecciona una sucursal para ver el reporte de uso" },
                { status: 400 }
            );
        }
        ...
        const report = await InventoryReportsService.getUsageReport(scope.branchId, startDate, endDate);
